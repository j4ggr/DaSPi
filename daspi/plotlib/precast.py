...
class ParameterRelevanceCharts(JointChart):
    ...
    ![ParameterRelevanceChartsDocstringExample](https://raw.githubusercontent.com/j4ggr/DaSPi/main/docs/img/precast_parameter-relevance-charts_example.png)
    """
    __slots__ = ('lm',)
    ...
    def label(self, info: bool | str = False, **kwds) -> Self:
...
class ResidualsCharts(JointChart):
    ...
    ![ResidualsChartsDocstringExample](https://raw.githubusercontent.com/j4ggr/DaSPi/main/docs/img/precast_residuals-charts_example.png)
    """
    __slots__ = ('lm',)
...
class PairComparisonCharts(JointChart):
    ...
    ![PairComparisonChartsDocstringExample](https://raw.githubusercontent.com/j4ggr/DaSPi/main/docs/img/precast_pair-comparison-charts_example.png)
    """
    ...
    def plot(self) -> Self: # type: ignore
        ...
        - ParallelCoordinate including a MeanTest and QuantileBoxes
        plot, to reflect the difference within the absolute values.
...
class BivariateUnivariateCharts(JointChart):
    ...
    ratios : list[float], optional
        ...
        height. If not provided or None, the default is [3, 1].
    stretch_figsize : bool | float | tuple[float, float], optional
        If True, the height and width of the figure are stretched based 
        on the number rows and columns in the axes grid. If a float is 
        provided, the figure size is stretched by the given factor. If a 
    ...
    ![BivariateUnivariateChartsDocstringExample](https://raw.githubusercontent.com/j4ggr/DaSPi/main/docs/img/precast_bivariate-univariate-charts_example.png)
    ...
    def __init__(
        ...
        _ratios = ratios or [3, 1]
        assert len(_ratios) == 2, (
            f'ratios must be a list of two floats, e.g. [4, 1], got {_ratios}')
...
class ProcessCapabilityAnalysisCharts(JointChart):
    ...
    def stripes(
        ...
        Self
            ...
            chaining.
        """
        def flags(x: bool) -> Generator[bool, Any]:
            yield from (x, False, x, False, False)
...
class GageRnRCharts(JointChart):
    ...
    def plot(self) -> Self:
        ...
        super().plot(
            ...
            show_center=True,
            on_last_axes=True,
            kw_call={'kw_center': {'marker': '_', 's': 10}})
...
class PairwiseMatrixCharts(JointChart):
    ...
    source : DataFrame
        The source data.
    features : tuple[str, ...]
        The features for which the pairwise matrix charts will be 
        created.
    hue : str, optional
    ...
    ![PairwiseMatrixChartsDocstringExample](https://raw.githubusercontent.com/j4ggr/DaSPi/main/docs/img/precast_pairwise-matrix-charts_example.png)
    """
    __slots__ = ('_diag_axes',)

    _diag_axes: tuple[Axes, ...]
    """A tuple containing the additional diagonal axes of the pairwise matrix 
    charts."""
    ...
    def __init__(
        ...
        n = len(features)
        targets = tuple(f for f in features for _ in range(n))
        features = tuple(f for _ in range(n) for f in features)
        equals = tuple(f == t for f, t in zip(features, targets))
        super().__init__(
            source=source,
            target=targets,
            feature=features,
        ...
        self._diag_axes = tuple(
            ax.twiny() for ax, eq in zip(self.axes, equals) if eq)
    
    @check_label_order
    def plot(
            self,
            *,
            diag_plot: type[Plotter] | None = GaussianKDE,
            lower_plot: type[Plotter] | None = LinearRegressionLine,
            upper_plot: type[Plotter] | None = GaussianKDEContour,
            kw_diag: dict[str, Any] | None = None,
            kw_lower: dict[str, Any] | None = None,
            kw_upper: dict[str, Any] | None = None,
            ) -> Self:
        """Plot the pairwise matrix charts.

        Parameters
        ----------
        diag_plot : type[Plotter] | None, optional
            The type of plot to use for the diagonal subplots. If None, 
            the diagonal subplots will be skipped. Default is 
            :class:`GaussianKDE`.
        lower_plot : type[Plotter] | None, optional
            The type of plot to use for the lower triangle subplots. If
            None, the lower triangle subplots will be skipped. Default 
            is :class:`LinearRegressionLine`.
        upper_plot : type[Plotter] | None, optional
            The type of plot to use for the upper triangle subplots. If
            None, the upper triangle subplots will be skipped. Default 
            is :class:`GaussianKDEContour`.
        kw_diag : dict[str, Any] | None, optional
            Additional keyword arguments to be passed to the plotter for
            the diagonal subplots. If None, default keyword arguments 
            will be used, which are
            :obj:`{'visible_spines': 'target', 'hide_axis': 'both'}`.
            Default is None.
        kw_lower : dict[str, Any] | None, optional
            Additional keyword arguments to be passed to the plotter for
            the lower triangle subplots. Default is None.
        kw_upper : dict[str, Any] | None, optional
            Additional keyword arguments to be passed to the plotter for
            the upper triangle subplots. Default is None.
        """
        if kw_diag is None:
            kw_diag = {
                'visible_spines': 'target',
                'hide_axis': 'both',}
        kw_lower = kw_lower or {}
        kw_upper = kw_upper or {}
        for row, diag_ax in zip(range(self.axes.shape[0]), self._diag_axes):
            for col in range(self.axes.shape[1]):
                kwds = {}
                plotter = SkipSubplot
                chart = self._next_chart_()
                if diag_plot is not None and row == col:
                    plotter = diag_plot
                    self.axes._ax = diag_ax
                    kwds = kw_diag
                elif lower_plot is not None and row > col:
                    plotter = lower_plot
                    kwds = kw_lower
                elif upper_plot is not None and row < col:
                    plotter = upper_plot
                    kwds = kw_upper
    ...
    def label(# type: ignore
            ...
            *,
            fig_title: str = '',
            sub_title: str = '',
            feature_label: bool | tuple[str, ...] = True,
            target_label: bool | tuple[str, ...] = True,
            info: bool | str = False,
            ) -> Self:
        """Add labels and titles to the chart.

        This method sets various labels and titles for the chart, 
        including figure title, subplot title and additional information.
        ...
        sub_title : str, optional
            The subtitle for the entire figure, by default ''.
        feature_label : bool | tuple[str, ...], optional
            The label for the feature variable (x-axis). If set to True, 
            the feature variable names will be used. If set to False or 
            None, no label will be added. If a tuple of strings is 
            provided, it will be used as the labels. Default is True.
        target_label : bool | tuple[str, ...], optional
            The label for the target variable (y-axis), by default ''. 
            If set to True, the target variable name will be used. 
            If set to False or None, no label will be added.
            If a tuple of strings is provided, it will be used as the 
            labels. Default is True.
        info : bool | str, optional
            Additional information to display on the chart. If True, 
            the date and user information will be automatically added 
            at the lower left corner of the figure. If a string is 
            provided, it will be shown next to the date and user, 
            separated by a comma. By default, no additional information 
            is displayed.
        """
        if feature_label is True:
            feature_label = self.features

        if target_label is True:
